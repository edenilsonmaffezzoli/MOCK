        ...
        try:
            ...
            if user is not None:
                ...
                if user.is_superuser:
                    user_type = 'admin'
                    redirect_url = '/admmin_home'
                elif hasattr(user, 'workers') and user.workers:
                    user_type = 'prestador'
                    redirect_url = '/workers_home'
                else:
                    user_type = 'cliente'
                    redirect_url = '/index'
                ...
                return JsonResponse({
                    ...
                    'message': f'Login realizado como {user_type.upper()}!'
                })
            else:
                return JsonResponse({
                    'success': False,
...
class home(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # Select random services, but handle cases where there are fewer than 3 services
        num_services_to_select = min(3, len(all_services))
        selected_services = random.sample(all_services, num_services_to_select) if all_services else []
